...
class CardArithmeticsTests(unittest.TestCase):
    ...
    def _fillShoeUp(self, game, letter):
        game.shoe.container = []
        for _ in range(200):
            game.shoe.container.append(Deck.get_card(letter))
    ...
    def testStandAction_won_chipsIncreased(self):
        ...
        self._fillShoeUp(game, '6')
        game.shoe.container.append(Deck.get_card('Q'))  # will be mine
        game.shoe.container.append(Deck.get_card('3'))  # will be dealer's
        game.shoe.container.append(Deck.get_card('J'))  # will be mine
        game.shoe.container.append(Deck.get_card('4'))  # will be dealer's

        result = game.simulate()

        self.assertEqual(result[0].total_number_of_chips, 10000)
        self.assertEqual(result[1].total_number_of_chips, 10002)
    ...
    def testStandAction_lost_chipsDecreased(self):
        ...
        self._fillShoeUp(game, '6')
        game.shoe.container.append(Deck.get_card('8'))  # will be mine
        game.shoe.container.append(Deck.get_card('3'))  # will be dealer's
        game.shoe.container.append(Deck.get_card('J'))  # will be mine
        game.shoe.container.append(Deck.get_card('4'))  # will be dealer's
        ...
        self.assertEqual(result[0].total_number_of_chips, 10000)
        self.assertEqual(result[1].total_number_of_chips, 10000 - 2)
